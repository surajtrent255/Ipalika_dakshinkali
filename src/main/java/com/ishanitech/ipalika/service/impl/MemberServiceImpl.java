...
//			begin
			if(roleWardDTO.getUrlObject().getGoBack()){
				if(roleWardDTO.getUrlObject().getPrevLastItemId() != 0){
					caseQuery += " AND fm.id < "+roleWardDTO.getUrlObject().getPrevLastItemId();
				} if(!roleWardDTO.getUrlObject().getSearchKey().trim().isEmpty() ){
					caseQuery += " AND fm.full_name LIKE '%"+roleWardDTO.getUrlObject().getSearchKey().trim()+"%'";
				...
				if(roleWardDTO.getUrlObject().getPageLimit() > 0){
					caseQuery += " ORDER BY fm.id DESC LIMIT "+roleWardDTO.getUrlObject().getPageLimit();
				} else {
					caseQuery += " ORDER BY a.id DESC LIMIT 10";
				}
			} else {
				caseQuery = CustomQueryCreator.generateQueryWithCase(request, PaginationTypeClass.FAMILY_MEMBERS);
