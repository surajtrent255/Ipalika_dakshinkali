...
public interface ReportDAO {
	...
	@SqlQuery("select answer_97 from answer where answer_3 =:wardNo and  length(answer_97) > 0;")
	List<String>  getIndividualWardVyabasaikReport(@Bind int wardNo);

	@SqlQuery("select answer_97 from answer where length(answer_97) > 0 and deleted = false;")
	List<String>  getTotalVyabasaikReport();
	default List<String> getVyabasaikReport(int wardNo){
		List<String> bReports;
		if(wardNo > 0){
			 bReports = getIndividualWardVyabasaikReport(wardNo);
		} else{
			bReports = getTotalVyabasaikReport();
		}
		return bReports;
