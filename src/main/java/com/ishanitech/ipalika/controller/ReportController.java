...
public class ReportController {
	...
	@GetMapping("/byabashahi/{wardNo}")
	public ResponseDTO<ByabasahikReportDTO> getByaBasayaReport(@PathVariable("wardNo") int wardNo){
		try{
			return new ResponseDTO<>(reportService.getAllByabashahiReport(wardNo));
		} catch(Exception ex){
			log.error("while fetching byabasaya report "+ ex.getMessage());
			throw new CustomSqlException(" something went wrong while fetching byabashahik report");
		}
	}
